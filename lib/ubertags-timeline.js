...
function fix_content_position() {
	// This will fix the position of the ubertag-content container, if it exists
...
function onLoad() {
    ...
    var overviewSource = new Timeline.DefaultEventSource(); // Overview events
	...
	function load_json1(json, url) {
		...
		// Load next band, detailed
		tl.loadJSON(json_data_url + "?type=detailed", load_json2);
	};
	...
	function load_json2(json, url) {
		...
		// Fix position of content container
		fix_content_position();
		
		
		tl.getBand(0).setCenterVisibleDate(overviewSource.getLatestDate());
		tl.getBand(1).setCenterVisibleDate(overviewSource.getLatestDate()); // Center the timline on last entry
	};
	...
	tl.loadJSON(json_data_url + "?type=overview", load_json1);
